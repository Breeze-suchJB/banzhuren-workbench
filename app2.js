...
function renderHelp() {
  ...
  const faq = [
    ...
    ['怎么绑定腾讯云 CloudBase 同步？', '① 打开 cloud.tencent.com 搜「云开发 CloudBase」→ 新建环境，<b>必须包含「文档型数据库」</b>（不要选 PostgreSQL / PG 模式，否则只有“新建表”没有“新建集合”，工作台用不了），复制「环境ID（envId）」。② 环境 → 登录授权 → 登录方式，开启「匿名登录」。③ 环境 → 安全配置 → 安全域名，把工作台网址加进去（本地调试填 localhost）。④ 数据库（文档型）→ 新建两个集合并把权限设为「所有用户可读写」：workbench_sync_meta、workbench_sync_chunk。⑤ 数据管理 → 云同步 → 同步方式选「腾讯云 CloudBase」→ 粘贴环境ID → “保存并立即同步”；其他设备填相同环境ID即可互通。'],
