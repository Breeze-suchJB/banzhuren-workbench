...
function renderHelp() {
  ...
  const mods = [
    ...
    ['🏫 班级管理', '维护班级：名称、年级、教室、任课角色（如“数学：郑老师”）。课表页“🔄 按班级角色同步教师”会把角色里的任课老师同步到课表；你在课表手动改过的老师优先保留。'],
    ['👥 学生管理', '增删改学生：姓名/学号/性别/住走/职务/家庭/家长1、家长2（每位含“关系：父亲/母亲/其他”）/小组/标签/预警。支持搜索联想（拼音首字母/姓名/学号）、考勤率/成绩/排名/积分表头升降序、导入导出 CSV。'],
    ['📅 日历假期', '管理假期安排、重要事项倒计时。'],
    ['📝 成绩管理', '新建考试 → 下载“空白模板” → 按模板填分 → “📥 导入 CSV”或手录 → 保存（自动算总分与排名）。成绩录入/个人成绩/班级成员成绩三处科目顺序一致（“编辑科目”可拖动/增删科目）；表头可升降序；学情分析里参考/及格/尖子/临界/不及格名单可点击查看具体学生。'],
    ['✅ 考勤管理', '日常点名（按日期逐人勾选）、离校登记、考勤统计；点击某生出勤率可查看该生哪天缺勤/请假。'],
    ['📚 作业管理', '布置作业、收缴/迟交/未交标记、背诵检查模板、作文评语批量导出。'],
    ['📄 请假管理', '登记请假、审批（批准/驳回）。'],
    ['🗂️ 班级事务', '积分、违纪、值日（一键轮换 / 教室值日 / 自动刷新名单）、座位（两人同桌、随机分配、自动适配人数、前排优先）、课表（科目设置、按角色同步教师）、班会。'],
    ['🏅 德育活动', '班级/教师荣誉墙；周末研学活动可拍照/上传留痕。'],
    ['📞 家校沟通', '联系记录、通知发布。'],
    ['⭐ 学生评价', '评语、五育评价。'],
    ['📌 工作管理', '待办四象限、工作日志（每条可拍照/上传留痕）、谈心谈话。'],
    ['🛡️ 安全特殊', '安全台账、重点学生等。'],
    ['🤖 智能助手', 'AI 文本工具：写评语、通知、总结等。'],
    ['🧮 学科工具', '备课、试卷、背书、资源、选科组合登记、生涯规划。'],
    ['💾 数据管理', '数据总览、系统设置（班级/科目/尖子临界不及格划线）、云同步（拉取间隔可调）、备份恢复、清空与重置。'],
    ['👤 个人资料', '教师姓名、头像、主题配色、显示偏好。']
  ];
  ...
  async function ensure() {
    ...
    if (loginErr) {
      ...
      if (low.indexOf('failed to fetch') >= 0 || low.indexOf('networkerror') >= 0 || low.indexOf('load failed') >= 0 || low.indexOf('cors') >= 0 || low.indexOf('network') >= 0) {
        /* 现场直连探测：立刻测一次接口是否可达，让报错自带真相 */
        let probe = '';
        try {
          const pr = await fetchTimeout('https://' + envId + '.' + region + '.tcb-api.tencentcloudapi.com/auth/v1/signin/anonymously', { method: 'OPTIONS', headers: { 'Origin': location.origin, 'Access-Control-Request-Method': 'POST' } }, 10000);
          probe = '【此刻直连探测：接口可达（HTTP ' + pr.status + '）→ 多半是刚才网络瞬时抖动，请直接再点一次“保存并立即同步”】';
        } catch (e2) { probe = '【此刻直连探测：也失败（' + errMsg(e2) + '）→ 确实是网络/设备到腾讯云接口不通，请改用 Supabase 同步】'; }
        throw new Error('CloudBase 请求被浏览器拦截或网络不通（CORS/跨域）：' + probe + '请在云开发控制台把工作台网址加入「安全域名」（环境 → 环境配置/设置 → 安全来源/安全配置 → 安全域名 → 添加域名：' + location.origin + '；免费版无此入口请用 CloudBase 静态托管默认网址打开），并确认网络正常、云同步卡片的「地域（region）」与你环境一致（当前 ' + region + '）。原始错误：' + em);
      }
